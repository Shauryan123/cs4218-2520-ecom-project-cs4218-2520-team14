...
test.describe("Order Retrieval and Display", () => {
  test("should display orders on page load", async ({ page }) => {
    ...
    await expect(page.locator(".border.shadow")).toHaveCount(ordersData.length);
  });
  ...
  test("should display correct order metadata", async ({ page }) => {
    ...
    for (const [index, order] of ordersData.entries()) {
      ...
      const paymentCell = orderContainer.locator("table tbody tr td").nth(4);
      ...
      const quantityCell = orderContainer.locator("table tbody tr td").nth(5);
      await expect(quantityCell).toContainText(String(order.products.length));
    }
  });
...
test.describe("Order Status Update", () => {
  test("should successfully update order status", async ({ page, request }) => {
